 ...
 * outside the bounds.
 */
public class CustomMenuBarSkin extends BehaviorSkinBase<MenuBar, BehaviorBase<MenuBar>> implements TraverseListener {
    ...
    private static WeakHashMap<Stage, Reference<CustomMenuBarSkin>> systemMenuMap;
    ...
    private static CustomMenuBarSkin getBasicMenuBarSkin(Stage stage) {
        if (systemMenuMap == null) return null;
        Reference<CustomMenuBarSkin> skinRef = systemMenuMap.get(stage);
    ...
    private static void setSystemMenu(Stage stage) {
        if (stage != null && stage.isFocused()) {
            while (stage != null && stage.getOwner() instanceof Stage) {
                CustomMenuBarSkin skin = getBasicMenuBarSkin(stage);
    ...
    public CustomMenuBarSkin(final MenuBar control) {
...
//        });
        ...
        Utils.executeOnceWhenPropertyIsNonNull(control.sceneProperty(), (Scene scene) -> {
            final BooleanProperty altDown = new SimpleBooleanProperty(false);
    ...
    private void rebuildUI() {
        ...
        if (Toolkit.getToolkit().getSystemMenu().isSupported()) {
            ...
            if (scene != null) {
                    ...
                    sceneChangeListener = (observable, oldValue, newValue) -> {
                        ...
                        if (oldValue != null) {
                            if (oldValue.getWindow() instanceof Stage) {
                                ...
                                final CustomMenuBarSkin curMBSkin = getBasicMenuBarSkin(stage);
                                if (curMBSkin == CustomMenuBarSkin.this) {
                ...
                // If the system menu references this CustomMenuBarSkin, then we're done with rebuilding the UI.
                // If the system menu does not reference this CustomMenuBarSkin, then the MenuBar is a child of the scene
                ...
                if (currentMenuBarStage != null ? getBasicMenuBarSkin(currentMenuBarStage) == CustomMenuBarSkin.this : getSkinnable().isUseSystemMenuBar()) {
                    return;
                }

            } else {
                // if scene is null, make sure this CustomMenuBarSkin isn't left behind as the system menu
                if (currentMenuBarStage != null) {
                    final CustomMenuBarSkin curMBSkin = getBasicMenuBarSkin(currentMenuBarStage);
                    if (curMBSkin == CustomMenuBarSkin.this) {
                        setSystemMenu(null);
                    }
                }
...
//            });
    ...
    private void cleanUpSystemMenu() {
        ...
        if (currentMenuBarStage != null && getBasicMenuBarSkin(currentMenuBarStage) == CustomMenuBarSkin.this) {
            setSystemMenu(null);
        }

        if (systemMenuMap != null) {
            Iterator<Map.Entry<Stage,Reference<CustomMenuBarSkin>>> iterator = systemMenuMap.entrySet().iterator();
            while (iterator.hasNext()) {
                Map.Entry<Stage,Reference<CustomMenuBarSkin>> entry = iterator.next();
                Reference<CustomMenuBarSkin> ref = entry.getValue();
                CustomMenuBarSkin skin = ref != null ? ref.get() : null;
                if (skin == null || skin == CustomMenuBarSkin.this) {
                    iterator.remove();
                }
            }
        }
    ...
    static class MenuBarButton extends MenuButton {
        ...
        public CustomMenuBarSkin getBasicMenuBarSkin() {
            return menuBarSkin;
        }
    ...
    private static final CssMetaData<MenuBar,Number> SPACING =
            new CssMetaData<MenuBar,Number>("-fx-spacing",
                ...
                public boolean isSettable(MenuBar n) {
                    final CustomMenuBarSkin skin = (CustomMenuBarSkin) n.getSkin();
