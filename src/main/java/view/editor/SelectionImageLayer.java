...
public class SelectionImageLayer extends EditorLayer {

    private Point drag;
    private BooleanProperty active = new SimpleBooleanProperty(false);
    private OutlineRect outlineRect;

    public SelectionImageLayer(int width, int height, PixelReader reader) {
    ...
    public void setOutlineRect(OutlineRect outlineRect) {
        ...
        outlineRect.translateXProperty().bind(translateXProperty().multiply(getParent().scaleXProperty()));
        outlineRect.translateYProperty().bind(translateYProperty().multiply(getParent().scaleXProperty()));
    }
